...
namespace InvoiceWorker.ComponentTests
{
...
";
            ...
            // Assert
            File.Exists($@"C:\InvoiceData-{@event.Content.InvoiceId}").Should().BeTrue();
            var result = await File.ReadAllTextAsync($@"C:\InvoiceData-{@event.Content.InvoiceId}");
